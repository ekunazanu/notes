Study of behaviour of structures and concepts following certain rules. The most fundamental rules or truths are assumptions, called [axioms](https://en.wikipedia.org/wiki/Axiom) — taken as truth, no proofs — used as a starting point for reasoning.

## Algebraic Structures

Mathematical operations are not limited to numbers. Algebraic structures provide a way to formalise behaviour of operations performed on abstract entities not limited to numbers — to generalise operations on numbers to operations on abstract structures. Different algebraic structures are defined based on chosen axioms, each with their own properties and behaviour resulting from the axioms. More generic structures include more entities, but may have lesser specific properties or behaviour. More specific or restrictive structures include fewer entities but may have more specific properties.

The abstract entities can be represented as collections of different things called *sets* and the individual entities are called *elements* of the set. The entities when operated on, can result in a well defined output. The operations on the entities, (called operands or arguments, in this context) and their outputs are usually denoted as $\omega:X_1 \times X_2 \ldots \times X_n \rightarrow Y$ where $\omega$ represents the operation, and $X_1, X_2 \ldots X_n$ represents the operands, and $Y$ is the output.
...
* E.g. $(\mathbb{Z}, +)$ is an abelian group. $(\mathbb{R}, \times)$ is an abelian group. $(\mathbb{Z}, -)$ is *not* a group.
...
* Or in short, a field is a commutative ring where $0 \neq 1$ and all non-zero elements are invertible under multiplication.
* E.g. $(\mathbb{R}, +, \cdot)$ is a field, and so is $(\mathbb{Q}, +, \cdot)$ and $(\mathbb{C}, +, \cdot)$, but $(\mathbb{Z}, +, \cdot)$ is *not* a field since multiplicative inverse does not exist for all elements $\in \mathbb{Z}$.

As evident, every field is a ring, and every ring is a group. The above are common algebraic structures, but [others](https://en.wikipedia.org/wiki/Algebraic_structure#Common_algebraic_structures) exist too.
...
Numbers that extends the real number system using $i = \sqrt{-1}$. Can be used to represent rotations on the complex plane. Usually represented as $z = a+bi$ where $a,b \in \mathbb{R}$ and $i = \sqrt{-1}$.
...
* **Complex Conjugate** — The reflection of complex number a across the real axis, denoted by $\overline{z}$.
...
* Input set is called domain, output set is called codomain.
* The output element is called the image.
* The set of all the possible inputs that maps to an output is called the preimage.
* The set containing all images for all elements of a domain is the called the range or image.
* Can be denoted as $f: X \rightarrow Y$ where $X$ is the domain, $Y$ is the codomain.
* Individual mappings can be denoted as $f(x) = y$ where $x \in X$ and $y \in Y$ of $f: X \rightarrow Y$.
...
* Inverse of a function exists $\iff$ If a function is is bijective.
...
* Functions can be nested, $g(f(x))$.
* Generally inner/right functions are evaluated before outer/left.
...
* For $y = \log(x)$ where $x < 0$, then  $y = \log(|x|) +k \cdot i\pi$ , where $k = 2n + 1, n \in \mathbb{N}$. Can be derived using Euler's formula. [Ref](https://www.youtube.com/watch?v=SYxyemNSSm8)

## Trigonometry

* Branch of mathematics that deals with angles.
* $\sin(x)$ is the ratio of the lengths of the side opposite that angle to the hypotenuse of a right triangle.
* $\cos(x)$ is the ratio of the lengths of the side adjacent that angle to the hypotenuse of a right triangle.

![image of sin and cos using a right triangle](trigometrictriangle.png)

* $\sin(x)$ and $\cos(x)$ are defined $f: \mathbb{R} \rightarrow [-1,1]$.
...
![unit circle with trig identities](https://upload.wikimedia.org/wikipedia/commons/e/e8/Trigonometric_functions_and_their_reciprocals_on_the_unit_circle.svg)
...
![trigonometric identity sign change due to shifts](https://upload.wikimedia.org/wikipedia/commons/8/85/Unit_Circle_-_shifts.svg)
 ...
 * [Linear Algebra](Mathematics/Linear%20Algebra.md)
 * [Calculus](Mathematics/Calculus.md)
 * [Probability Theory](Mathematics/Probability%20Theory.md)
 * [Graph Theory](Mathematics/Graph%20Theory.md)
 * [Group Theory](Mathematics/Group%20Theory.md)
 * [Bezier Curves](Mathematics/Bezier%20Curves.md)
 * [Convolution](Mathematics/Convolution.md)